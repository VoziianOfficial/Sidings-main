...
(function () {
  ...
  document.querySelectorAll('[data-align]').forEach((button) => { button.type = 'button'; button.addEventListener('click', () => { const machine = document.querySelector('.align-machine'); machine?.classList.add('aligned'); window.setTimeout(() => machine?.classList.remove('aligned'), 1800); }); });
  document.querySelectorAll('[data-style]').forEach((button) => { button.type = 'button'; button.setAttribute('aria-pressed', String(button.classList.contains('active'))); button.addEventListener('click', () => { setActive('[data-style]', button); const morph = document.querySelector('.style-morph'); if (morph) morph.dataset.style = button.dataset.style; }); });
  document.querySelectorAll('[data-symptom]').forEach((button) => { button.type = 'button'; button.setAttribute('aria-pressed', String(button.classList.contains('active'))); button.addEventListener('click', () => { const network = document.querySelector('.network'); if (network) network.dataset.symptom = button.dataset.symptom; setActive('[data-symptom]', button); }); });
  document.querySelectorAll('[data-threshold]').forEach((button) => { button.type = 'button'; button.addEventListener('click', () => { const value = { single: ['10%', '8%'], cluster: ['30%', '32%'], section: ['49%', '56%'], wide: ['72%', '78%'] }[button.dataset.threshold]; const threshold = document.querySelector('.threshold'); if (!threshold || !value) return; threshold.style.setProperty('--damage-size', value[0]); threshold.style.setProperty('--meter', value[1]); }); });
  document.querySelectorAll('[data-moisture]').forEach((button) => { button.type = 'button'; button.addEventListener('click', () => { document.querySelector('.moisture')?.classList.toggle('active'); }); });
  document.querySelectorAll('[data-match]').forEach((button) => { button.type = 'button'; button.addEventListener('click', () => { const lab = document.querySelector('.match-lab'); if (!lab) return; const count = Number(lab.dataset.count || 0) + 1; lab.dataset.count = String(count); if (count >= 3) lab.classList.add('matched'); }); });
